    ...
    <div className="relative min-h-screen bg-slate-50 text-slate-800 font-sans overflow-hidden selection:bg-teal-300 selection:text-white">
      ...
      <div className="relative z-10 max-w-[1600px] mx-auto px-6 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-10 gap-4">
          <h1 className="text-3xl font-extrabold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-slate-800 to-slate-500">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="flex items-center gap-2 bg-white/40 backdrop-blur-md border border-white/60 shadow-sm px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-white/60 transition-all">
...
function ProductCard({ product }) {
  return (
    <div className="group flex flex-col relative bg-white/30 backdrop-blur-xl border border-white/50 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_20px_40px_rgb(0,0,0,0.12)] hover:-translate-y-2 transition-all duration-500 ease-out rounded-[2rem] p-4">
      
      {/* Image Container */}
      <div className="relative aspect-square w-full bg-white/40 rounded-2xl overflow-hidden mb-5 flex items-center justify-center">
        {/* New Arrival Tag */}
      ...
      <div className="flex flex-col flex-1 px-2 pb-2">
        ...
        <h3 className="font-bold text-slate-800 text-sm mb-3 line-clamp-2 leading-relaxed" title={product.title}>
          {product.title}
        </h3>
        
        <div className="mt-auto flex justify-between items-center">
          <span className="text-lg font-extrabold bg-clip-text text-transparent bg-gradient-to-r from-teal-600 to-emerald-600">
            ${product.price.toFixed(2)}
          </span>
          <span className="text-xs font-semibold text-rose-400 bg-rose-50 px-2 py-1 rounded-md">
